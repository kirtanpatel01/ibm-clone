...
function NextSteps() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 mx-0 sm:mx-8 2xl:mx-0 mb-32">
      <h1 className="mx-4 sm:mx-0 text-4xl lg:text-5xl xl:text-6xl sm:font-light">
        Take the next step
      </h1>
      ...
      {list.map((item) => (
        <li
          key={item.title}
          className="sm:flex border p-0 sm:p-4 gap-8 mt-0 lg:mt-20"
        >
          <img
            src={item.img}
            alt={item.img}
            className="w-full sm:w-[50%] lg:w-80"
          />
